...
public class SequentialArrangerEditorViewModel : ArrangerEditorViewModel, IMouseCaptureProxy {
	...
	public override void SaveChanges() => IsModified = false;
	...
	public void NewScatteredArrangerFromSelection() {
		if (SnapMode == SnapMode.Element) {
			var x = Selection.SelectionRect.SnappedLeft / WorkingArranger.ElementPixelSize.Width;
			var y = Selection.SelectionRect.SnappedTop / WorkingArranger.ElementPixelSize.Height;
			var width = Selection.SelectionRect.SnappedWidth / WorkingArranger.ElementPixelSize.Width;
			var height = Selection.SelectionRect.SnappedHeight / WorkingArranger.ElementPixelSize.Height;
	...
	public void ApplyDefaultElementLayout() => ChangeElementLayout(_layoutService.DefaultElementLayout);
	...
	public void ChangeElementLayout(string layoutName) => ChangeElementLayout(_layoutService.ElementLayouts[layoutName]);
	...
	public override void Undo() => throw new NotSupportedException("Sequential Arrangers are read-only");

	public override void Redo() => throw new NotSupportedException("Sequential Arrangers are read-only");

	public override void ApplyHistoryAction(HistoryAction action) => throw new NotSupportedException("Sequential Arrangers are read-only");

	public override void ApplyPaste(ArrangerPaste paste) => throw new NotSupportedException("Sequential Arrangers are read-only");
	#endregion
}
