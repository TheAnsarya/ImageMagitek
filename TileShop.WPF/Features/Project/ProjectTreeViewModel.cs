...
public class ProjectTreeViewModel : ToolViewModel, IDropTarget, IHandle<AddScatteredArrangerFromCopyEvent> {
	...
	public void AddNewFolder(ResourceNodeViewModel parentNodeModel) => _projectService.CreateNewFolder(parentNodeModel.Node, "New Folder").Switch(
			success => {
	...
	public void AddNewPalette(ResourceNodeViewModel parentNodeModel) {
		...
		if (_windowManager.ShowDialog(dialogModel) is true) {
			var pal = new Palette(dialogModel.PaletteName, _paletteService.ColorFactory,
				...
				DataFile = dialogModel.SelectedDataFile
			};
	...
	public void ExploreResource(ResourceNodeViewModel nodeVM) => _diskExploreService.ExploreDiskLocation(nodeVM.Node.DiskLocation);
