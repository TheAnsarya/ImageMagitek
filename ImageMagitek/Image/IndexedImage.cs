...
public sealed class IndexedImage : ImageBase<byte> {
	...
	public override void Render() {
		...
		foreach (var (X, Y) in locations) {
			...
			if (el is ArrangerElement element && element.Codec is IIndexedCodec codec) {
				...
				var minX = Math.Clamp(element.X1, Left, Right - 1);
				var maxX = Math.Clamp(element.X2, Left, Right - 1);
				var minY = Math.Clamp(element.Y1, Top, Bottom - 1);
				var maxY = Math.Clamp(element.Y2, Top, Bottom - 1);
				var deltaX = minX - element.X1;
				var deltaY = minY - element.Y1;
	...
	public MagitekResult TrySetPalette(int x, int y, Palette pal) {
		...
		if (el is ArrangerElement element) {
			...
			for (var pixelY = element.Y1; pixelY <= element.Y2; pixelY++) {
				for (var pixelX = element.X1; pixelX <= element.X2; pixelX++) {
