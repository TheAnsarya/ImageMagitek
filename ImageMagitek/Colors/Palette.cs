...
public class Palette : IProjectResource {
	...
	/// </summary>
	private ColorRgba32[] NativePalette => _nativePalette.Value;
	...
	/// </summary>
	private IColor[] ForeignPalette => _foreignPalette.Value;
	...
	/// </summary>
	/// <returns></returns>
	public bool Reload() => LazyLoadPalette(DataFile, ColorModel, ZeroIndexTransparent);
	...
	public bool TryGetIndexByNativeColor(ColorRgba32 color, ColorMatchStrategy matchStrategy, out byte index) {
		if (matchStrategy == ColorMatchStrategy.Exact) {
			...
			for (byte i = 0; i < Entries; i++) {
				...
				var Distance = h1.Compare(h2, _comparator);
