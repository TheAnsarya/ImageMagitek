...
public struct FileBitAddress : IEquatable<FileBitAddress> {
	...
	public override int GetHashCode() => HashCode.Combine(BitOffset, FileOffset);
	...
	public static FileBitAddress operator +(FileBitAddress Address1, FileBitAddress Address2) {
		var bits = Address1.Bits() + Address2.Bits();
	...
	/// <returns></returns>
	public static FileBitAddress operator -(FileBitAddress Address, long Offset) {
		var addressbits = Address.Bits();
		var retbits = addressbits - Offset;

		return new FileBitAddress(retbits);
	}

	public static bool operator <(FileBitAddress Address1, FileBitAddress Address2) => Address1.Bits() < Address2.Bits();
